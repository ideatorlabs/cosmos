...
| **Open Knowledge Format** | The ledger is a conformant OKF v0.2 bundle (Google's open spec for knowledge as markdown with YAML frontmatter): every note carries `type`, `title`, `description`, `generated`, `verified`, `status` and `stale_after`, with `index.md` and `log.md` at the root. Any OKF consumer can read it; cosmos's own retrieval is BM25 with an evidence-path boost, no vectors. |
