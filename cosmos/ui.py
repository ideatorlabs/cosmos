...
const OPENLIKE=['open','claimed','pr_open','regressed'];
const flareAgeDays=m=>{const d=(m.meta&&(m.meta.status_at||m.meta.imported_on))||m.created;return d?Math.floor((Date.now()-new Date(d).getTime())/86400000):0};
const agingFlares=(days)=>findings().filter(m=>OPENLIKE.includes(fstatus(m))&&(m.meta||{}).severity!=='note'&&flareAgeDays(m)>=(days||14));
...
function dreams(){
 $('#subtitle').textContent='';
 const recallCard=()=>{const ms=S.dreams.filter(x=>x.recall_at_5!=null||x.edit_hit!=null);if(!ms.length)return '';const L=ms[0],P=(ms[1]&&(ms[1].eval_v||1)===(L.eval_v||1))?ms[1]:null;const pct=v=>v==null?'–':Math.round(v*100)+'%';const dv=(a,b)=>a==null||b==null?'':`<span class="small ${a<b-0.049?'bad':'muted'}">${a>=b?'+':''}${Math.round((a-b)*100)} vs previous</span>`;return `<div class="card" style="margin:14px 0"><h3 style="margin:0 0 6px">Does the right fact reach the agent?</h3><div class="small muted" style="margin-bottom:10px">Measured by every dream that changed the memory, on this repository's own facts and code.</div><div class="row" style="gap:28px"><div><div style="font-size:26px">${pct(L.edit_hit)}</div><div class="small">before an edit: the fact about the code being changed is shown</div>${dv(L.edit_hit,P&&P.edit_hit)}</div><div><div style="font-size:26px">${pct(L.recall_at_5)}</div><div class="small">recall@5: questions and file lookups</div>${dv(L.recall_at_5,P&&P.recall_at_5)}</div></div>${L.eval_drop?`<div class="callout" style="margin-top:10px">⚠ ${esc(L.eval_drop)} — look at what the last dream changed.</div>`:''}</div>`};
...
function verdicts(){
 ...
 const stale=S.memories.filter(m=>m.status==='stale-candidate');const nh=needsHuman();const aging=agingFlares(14);
 $('#subtitle').textContent='';
...
# nothing to commit: the dream commits .cosmos/ to your branch; it goes out when you push</pre>
...
cosmos obsidian --vault ~/Obsidian/Team  # link several repos' ledgers into one vault</pre>
 ...
 ['cli','12 · Command reference',`
  ...
  <tr><td><code>cosmos sync [--inline]</code></td><td>commit the team memory now (dreams and the watcher do it for you) · move a separate ledger branch into this branch</td></tr>
