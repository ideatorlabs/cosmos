...
cosmos obsidian --vault ~/Obsidian/Team  # link several repos' ledgers into one vault</pre>
 ...
 ['byitself','14 · What happens by itself',`
  ...
  <tr><td><b>Recall before an edit</b> — open flares, constraints and rules attached to the file the agent is about to change, or to a folder that contains it; explicit rules first</td><td>once per file per session</td><td>the agent's context (≈30 tokens per fact; full note via <code>cosmos_why</code>)</td></tr>
