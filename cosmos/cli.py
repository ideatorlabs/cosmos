...
def cmd_init(a) -> int:
    ...
    branch_note = ""
    if getattr(a, "branch", False) and (root / ".git").exists():
        # opt-in: the ledger on its own `cosmos` branch, pushed by itself
        if (root / ".cosmos").exists() and not _sync.is_branch_mode(load_config(root)):
            ok, msg = _sync.migrate(root)
        else:
            ok, msg = _sync.attach(root)
        branch_note = "ledger lives on the `cosmos` branch (.cosmos/ is a worktree of it)" if ok else f"could not attach the ledger branch: {msg}"
        _sync.ensure_ignored(root)
    elif (root / ".cosmos" / ".git").is_file():
        ok, msg = _sync.to_inline(root)
        branch_note = "the ledger now lives in this branch (.cosmos/ is committed here and travels with every branch made from it)" if ok else f"could not move the ledger into this branch: {msg}"
    ...
    if not _sync.is_branch_mode(cfg):
        _sync.write_inline_files(root)                 # .cosmos/state/ stays local; append-only files merge by union
        _sync.unignore(root)
...
def cmd_sync(a) -> int:
    """Commit the ledger now (normally done for you by dreams and the watcher)."""
    from . import sync as _sync
    cfg = load_config(); _require(cfg)
    if getattr(a, "inline", False):
        ok, msg = _sync.to_inline(cfg.paths.root)
        print(col("✓" if ok else "✗", "g" if ok else "r"), msg); return 0 if ok else 1
    if not _sync.is_branch_mode(cfg):
        c = _sync.commit_inline(cfg.paths.root, a.message or "cosmos: team memory")
        print(col("✓", "g"), ("committed .cosmos/ to this branch" if c else "nothing to commit") + " · it goes out when you push the branch"); return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s = sp.add_parser("init", help="set up .cosmos/, hooks, CLAUDE.md/AGENTS.md block, Obsidian vault"); s.add_argument("path", nargs="?"); s.add_argument("--command", help="hook command override"); s.add_argument("--no-vendor", action="store_true", help="don't vendor cosmos into .cosmos/vendor (teammates must pip install)"); s.add_argument("--no-user-hooks", action="store_true", help="do not touch ~/.claude/settings.json"); s.add_argument("--no-seed", action="store_true", help="do not read past sessions or start the first dream"); s.add_argument("--branch", action="store_true", help="keep the ledger on its own `cosmos` branch instead of in this branch"); s.add_argument("--no-branch", action="store_true", help=argparse.SUPPRESS); s.set_defaults(fn=cmd_init)
